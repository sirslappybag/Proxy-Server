...
{	
	...
	int recv_msg_size, bl, b2;
	int send_len,bytes_sent, flag;
	char buf[BUF_SIZE], code[80], servAdd[40], ipAdd[40], modBuf[BUF_SIZE];
	int select_ret;
	 ...
	 {     bytes_received=recv(sock_recv,buf,BUF_SIZE,0);		//read in information from the browser (client)		
		...
		{	strcpy(modBuf,code);
			...
			//printf("\nSERVER ADDRESS: %s\n", servAdd);
			char *tokenServer = NULL;
			tokenServer = strcat(servAdd, "\n");	//function call for server address (servAdd)~~~~~~~~~~~~~~~~~~
			//to check up the blacklist
			bl = checkBList(tokenServer);
			...
			char* tmp4 = strdup(servAdd);			//duplicate the line for strtok
			char* toke;				//declare the token variable
			toke = strtok(tmp4, "\n");			//pull out the protocol code
			strcpy(servAdd, toke);
			free(tmp4);
			//printf("\nADDER %s\n", servAdd);
		...
		//filter for language!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		//b2 = checkFilter(buf);
		if (b2 == 1)				//they're trying to look at a site with inappropriate language
		{	printf("\n\nThis site has inappropriate language!!\n\n");
				//send bad request protocol?????
				char* tmp3 = strdup(code);			//duplicate the line for strtok
				const char* token;				//declare the token variable
				token = strtok(tmp3, "\r");			//pull out the protocol code
		...
		sent = 0;
		send_len=strlen(buf);
		//printf("\nMade it TO the do loop\n");
		do {
			//printf("Made it IN the do loop\n");
			bytes_sent=send(sock_recv,buf,send_len,0);			//send reply to browser (client)
			if (bytes_sent < 0)
		...
		//printf("out of doWhile loop successfully\n");
		}
		fclose(fp);
...
int checkFilter(char *buffer)
{	printf("\nMADE IT!!!\n");

	char word[80];
	char line[SMALL_BUF];
	char* tmp = strdup(buffer);			//duplicate the line for strtok
	const char* tok;				//declare the token variable
	tok = strtok(tmp, " _-;<>:/\n\r");		//pull out a word
	strcpy(word, tok);	

	FILE *badWords = fopen("badWords.txt", "r");
	if (badWords == NULL)			//error opening the file
		return 0;
	if (!badWords)				//if file doesn't exist
		return 0;

	while (tok != NULL)				//until the buffer is empty
	{	while(fgets(line, SMALL_BUF, badWords) != NULL)		//until the list is finished
		{	if (strcmp(line, word) == 0)
			{	printf("\nFound %s!!!\n", line);
				fclose(badWords);
				return 1;
			}
		}tok = strtok(NULL, " _-;<>:/\n\r");	//pull out next word
		strcpy(word, tok);
	}
